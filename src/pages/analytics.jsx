...
const AnalyticsPage = ({ credits: initialCredits, consumptions: initialConsumptions }) => {
  ...
  const creditUtilization = totalCreditAmount > 0 ? ((totalCreditAmount - totalRemainingAmount) / totalCreditAmount) * 100 : 0;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <AnalyticsStatsGrid
        stats={[
          { title: "Capital Total Otorgado", value: `$${totalCreditAmount.toLocaleString()}`, icon: <DollarSign className="h-4 w-4 text-muted-foreground" />, description: "Suma de todos los límites de crédito." },
          { title: "Saldo Total Pendiente", value: `$${(totalCreditAmount - totalRemainingAmount).toLocaleString()}`, icon: <DollarSign className="h-4 w-4 text-muted-foreground" />, description: "Total que los clientes deben actualmente." },
          { title: "Créditos Activos", value: activeLoans, icon: <Users className="h-4 w-4 text-muted-foreground" />, description: "Clientes con créditos actualmente." },
          { title: "Utilización de Crédito", value: `${creditUtilization.toFixed(1)}%`, icon: <Percent className="h-4 w-4 text-muted-foreground" />, description: "Porcentaje del crédito total utilizado." }
        ]}
      />
